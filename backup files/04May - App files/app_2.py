...
defaults = {
    ...
    'gemini_model': "gemini-1.5-flash-latest", # Updated default model (using latest flash)
...
def reset_session():
    ...
    current_model = st.session_state.get('gemini_model', "gemini-1.5-flash-latest") # Use updated default
...
def main():
    ...
    with st.sidebar:
        ...
        # Model Selection
        model_options = [
             "gemini-1.5-flash-latest", # Recommended Flash
             "gemini-1.5-pro-latest",  # Recommended Pro
             "Custom"
        ]
    ...
    if not st.session_state.project_initialized:
                ...
                if not st.session_state.gemini_api_key:
                    ...
                    with st.spinner("Processing uploaded files..."):
                        ...
                        if success_count > 0:
                            ...
                            for name in st.session_state.dataframes.keys(): file_summary += f"- Tabular: {name}\n"
        ...
        if active_step == 0:
            ...
            if st.session_state.manager_plan:
                ...
                # --- Consultation & Review Expanders --- (Simplified placeholders)
                # with st.expander("Consult AI Manager"): st.info("Consultation feature placeholder")
                # with st.expander("Request Review from Director"): st.info("Reviewer feature placeholder")
                    ...
                    for file_name, df in st.session_state.dataframes.items():
                        ...
                        if profile:
                            ...
                            if isinstance(missing_summary, pl.DataFrame) and not missing_summary.is_empty():
                                ...
                                st.dataframe(missing_summary) # Display as Pandas
            ...
            if st.button("🤖 Generate Analysis Code & Insights", key="execute_task_btn"):
                if not task_to_run:
                    ...
                    try:
                        ...
                        data_sample_json = df_pl.head(5).write_json(row_oriented=True)
                    except Exception as e:
