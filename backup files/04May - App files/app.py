...
"""
            ...
            st.title("🔍 AI Associate - Analysis Guidance")

            if st.session_state.associate_guidance is None:
                if st.session_state.analyst_summary: # Ensure analyst summary exists
                    with st.spinner("AI Associate is generating analysis guidance..."):
                        # Prepare the prompt for the Associate
                        associate_prompt = st.session_state.associate_prompt_template.format(
                            problem_statement=st.session_state.problem_statement,
                        ...
                        if associate_response and not associate_response.startswith("Error:"):
                            st.session_state.associate_guidance = associate_response
                            add_to_conversation("associate", associate_response)
                        elif associate_response:
                             st.error(f"Associate Error: {associate_response}")
                        else:
                            st.error("Failed to get response from Associate.")
                else:
                    st.warning("Analyst summary is not available. Please complete Step 3 first.")
                    # Optionally, add a button to go back
                    if st.button("Go back to Data Understanding"):
                        st.session_state.current_step = 2
                        st.rerun()
            ...
            if st.session_state.associate_guidance:
                ...
                if st.button("Continue to Analysis Execution"):
                    st.session_state.current_step = 4 # Move to step 5 (index 4)
                    st.rerun()
